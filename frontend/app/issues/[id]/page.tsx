...
const RISK_LABELS: Record<string, { label: string; icon: string; desc: string }> = {
  ...
  misinformation: { label: "Misinformation",   icon: "💼", desc: "Fraud, scam, fake profile" },
};
...
export default function IssuesDetailPage() {
  ...
  const [report, setReport] = useState<MonitoringReport |Issue | null>(null);
  const [loading, setLoading] = useState(true);
  const [tab, setTab] = useState<"overview" | "details" | "flags">("overview");
  const [elapsed, setElapsed] = useState(0);
  const [assessing, setAssessing] = useState(false);

  const handleAssess = async (status: "relevant" | "irrelevant") => {
    if (!report) return;
    setAssessing(true);
    try {
  ...
  const handleDownload = async () => {
    ...
    doc.text("Strapping Media",margin+34,22);
    doc.setFont("helvetica","normal"); doc.setFontSize(9); doc.setTextColor(100,116,139);
    doc.text("Media Intelligence Monitoring Platform",margin+34,29);

    // Report label
    doc.setFont("helvetica","bold"); doc.setFontSize(10); doc.setTextColor(180,190,210);
    doc.text("BACKGROUND CHECK REPORT",W-margin,20,{align:"right"});
    ...
    doc.setFont("helvetica","bold"); doc.setFontSize(14); doc.setTextColor(255,255,255);
    doc.text(issue.keyword.charAt(0).toUpperCase(),margin+18,y+24,{align:"center"});

    // Name & info
    doc.setFont("helvetica","bold"); doc.setFontSize(14); doc.setTextColor(10,20,40);
    doc.text(issue.keyword,margin+34,y+13);
    doc.setFont("helvetica","normal"); doc.setFontSize(8.5); doc.setTextColor(70,90,115);
    doc.text(`✉  ${issue.keyword}`,margin+34,y+21);
    doc.text(`☎  ${"" ?? "-"}`,margin+34,y+28);
    doc.setFontSize(7.5); doc.setTextColor(140,155,175);
    doc.text(`Issue | nullID: ${issue.id}`,margin+34,y+35);
    ...
    const cats = [
      ...
      { label:"Misinformation", key:"misinformation", rgb:[239,68,68]   as [number,number,number], emoji:"💼" },
    ];
    ...
    if (flagged.length===0) {
      ...
      doc.text("Issue ini lolos monitoring tanpa flag.",margin+22,y+13.5);
      y+=22;
    } else {
    ...
    if (report.assessment_status) {
      sectionTitle("KEPUTUSAN HR ASSESSMENT");
      checkY(28);
    ...
    for (let i=1;i<=pageCount;i++) {
      ...
      doc.text("Strapping Media Media Intelligence Platform  ·  Dokumen Rahasia & Terbatas",margin,290);
      ...
      doc.text("HRCHECK CONFIDENTIAL",W/2,160,{align:"center",angle:45});
      doc.setGState(new (doc as any).GState({opacity:1}));
    }

    doc.save(`Strapping Media_${issue.keyword.replace(/\s+/g,"_")}_${new Date().toISOString().slice(0,10)}.pdf`);
  };


  const load = useCallback(async () => {
    try {
      const [c, r] = await Promise.all([api.getIssue(id), api.getReport(id)]);
      setIssue(c);
      setReport(r);
    } catch {
      try { setIssue(await api.getIssue(id)); } catch {}
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 20px 0" }}>
            ...
            <div style={{
              ...
              <div>
                <p style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 16, color: "var(--text)", marginBottom: 6 }}>
                  AI Monitoring Berjalan
                </p>
                <p style={{ fontSize: 13, color: "var(--text3)", lineHeight: 1.6, maxWidth: 420 }}>
                  Sistem sedang menganalisis profil sosial media issue. Proses ini biasanya memakan waktu 1–3 menit. Halaman akan otomatis diperbarui.
                </p>
              </div>
              ...
              <div style={{
                ...
                {report.assessment_status ? (
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{
                        ...
                        background: report.assessment_status === "relevant" ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.12)",
                        border: `1px solid ${report.assessment_status === "relevant" ? "rgba(34,197,94,0.3)" : "rgba(239,68,68,0.25)"}`,
                      }}>
                        {report.assessment_status === "relevant"
                      ...
                      <div>
                        <p style={{
                          ...
                          color: report.assessment_status === "relevant" ? "#22c55e" : "#ef4444",
                          marginBottom: 3,
                        }}>
                          {report.assessment_status === "relevant" ? "✅ Relevant" : "❌ Irrelevant"}
                        </p>
                  ...
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    ...
                    <button
                      disabled={assessing}
                      onClick={() => handleAssess("irrelevant")}
                      style={{
                        display: "flex", alignItems: "center", gap: 8, padding: "10px 20px",
                ...
                <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 18, padding: "22px 24px", boxShadow: "var(--sh-sm)" }}>
                  ...
                  {flagged.length === 0 ? (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, padding: "28px 0", color: "var(--text3)" }}>
                      ...
                      <p style={{ fontSize: 13 }}>Issue ini lolos monitoring tanpa flag.</p>
                    </div>
                  ) : (
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      {flagged.map((f, i) => {
                        const platformColor: Record<string, string> = {
                          ...
                          tiktok: "#69C9D0", "google search": "#4285f4", "berita media": "#f59e0b",
          ...
          {report?.status === "failed" && (
            <div style={{
              ...
              <p style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 16, color: "var(--text)" }}>Monitoring Gagal</p>
              <p style={{ fontSize: 13, color: "var(--text3)" }}>{report.error_message ?? "Terjadi kesalahan saat proses monitoring."}</p>
              <button onClick={load} style={{
