...
export default function DashboardPage() {
  ...
  const hasFetched            = useRef(false); // ← cegah double-fetch
    ...
    (async()=>{
      try {
        ...
        const withReports = await Promise.all(
          issues.map(async x => {
            try { return { ...x, report: await api.getReport(x.id) }; }
            catch { return x; }
          })
        );
        setRows(withReports);
  ...
  const highRiskIssues = rows.filter(r=>r.report?.overall_risk==="high"||r.report?.overall_risk==="critical");
  const mediumRiskIssues = rows.filter(r=>r.report?.overall_risk==="medium");
  const goodIssues     = rows.filter(r=>r.report?.overall_risk==="low");
  ...
  const CARDS = [
    ...
    { label:"Selesai",        val:completed,               sub:"monitoring selesai",       icon:UserCheck,  style:{ background:"linear-gradient(135deg,#00c896 0%,#009e76 100%)", border:"none" }, iStyle:{ background:"rgba(255,255,255,0.2)", color:"#fff" }, vColor:"#fff", href:"/issues?status=completed" },
    { label:"High / Kritis",  val:risk.high+risk.critical, sub:`${highPct}% dari total`,  icon:ShieldAlert, style:{ background:"linear-gradient(135deg,#ef4444 0%,#b91c1c 100%)", border:"none" }, iStyle:{ background:"rgba(255,255,255,0.2)", color:"#fff" }, vColor:"#fff", href:"/issues?risk=high" },
    { label:"Low Risk",       val:risk.low,                sub:"aman dilanjutkan",         icon:TrendingUp,  style:{ background:"linear-gradient(135deg,#3b82f6 0%,#1d4ed8 100%)", border:"none" }, iStyle:{ background:"rgba(255,255,255,0.2)", color:"#fff" }, vColor:"#fff", href:"/issues?risk=low" },
  ];
      ...
      <div style={{ padding:"36px 32px", maxWidth:1280, margin:"0 auto" }}>
        ...
        <div className="fade-up" style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:36, flexWrap:"wrap", gap:16 }}>
          <div>
            ...
            <p style={{ fontSize:13.5, color:"var(--text3)", marginTop:8 }}>Pantau aktivitas media monitoring secara real-time</p>
          </div>
          <Link href="/issues/add" className="btn btn-primary fade-up d2">
            <Plus size={14} /> Tambah Issue
          </Link>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:24 }}>
          {CARDS.map((c,i)=>(
            <Link key={c.label} href={c.href} style={{ textDecoration:"none" }}>
              <div className={`fade-up d${i+1}`} style={{ ...c.style as any, borderRadius:16, padding:"16px", position:"relative", overflow:"hidden", cursor:"pointer", transition:"transform 0.15s, box-shadow 0.15s" }}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"2fr 3fr", gap:20, marginBottom:20 }}>
          <div className="card fade-up d1" style={{ padding:"26px 24px" }}>
            ...
            {total===0
              ? <div style={{ display:"flex", flexDirection:"column", alignItems:"center", padding:"36px 0", gap:8 }}>
                  ...
                  <p style={{ fontSize:12.5, color:"var(--text3)" }}>Belum ada data</p>
                  <Link href="/issues/add" style={{ fontSize:12, color:"var(--accent)", fontWeight:600 }}>Mulai monitoring →</Link>
                </div>
          ...
          <div className="card fade-up d2" style={{ padding:"26px 24px" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
              ...
              {completed>0 && <span style={{ fontSize:11, color:"var(--text3)" }}>{completed} issue</span>}
            </div>
            {completed===0
              ? <div style={{ display:"flex", flexDirection:"column", alignItems:"center", padding:"36px 0", gap:8 }}>
                  <Activity size={20} style={{ color:"var(--text4)" }} />
                  <p style={{ fontSize:12.5, color:"var(--text3)" }}>Belum ada monitoring selesai</p>
                </div>
                      ...
                      <div key={cat.key}>
                        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:7 }}>
                          ...
                          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                            ...
                            <span style={{ fontSize:9.5, fontWeight:700, padding:"2px 7px", borderRadius:999, background:tagBg, color:tagColor }}>{tag}</span>
                          </div>
                        </div>
        ...
        {(risk.high+risk.critical)>0 && (
          <div className="fade-up" style={{ display:"flex", alignItems:"center", gap:16, padding:"18px 24px", borderRadius:16, marginBottom:20, background:"var(--danger-d)", border:"1px solid rgba(239,68,68,0.18)" }}>
            <AlertTriangle size={18} style={{ color:"var(--danger)", flexShrink:0 }} />
            <div style={{ flex:1 }}>
              <p style={{ fontSize:13.5, fontWeight:700, color:"var(--danger)" }}>{risk.high+risk.critical} issue memerlukan perhatian segera</p>
              <p style={{ fontSize:12, color:"var(--text3)", marginTop:2 }}>Tinjau hasil monitoring sebelum melanjutkan proses.</p>
            </div>
            <Link href="/issues" style={{ fontSize:12.5, fontWeight:700, color:"var(--danger)", textDecoration:"none" }}>Tinjau <ArrowRight size={12} /></Link>
          </div>
        )}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:20, marginBottom:24 }}>
          <div className="card fade-up d2" style={{ padding:"24px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:20 }}>
              ...
              <div>
                ...
                <p style={{ fontSize:12, color:"var(--text3)", marginTop:2 }}>Perlu penanganan segera</p>
              </div>
            </div>
            {highRiskIssues.length===0
              ? <p style={{ fontSize:13, color:"var(--text3)" }}>Tidak ada issue berisiko tinggi</p>
              : <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  {highRiskIssues.map(row=>(
                    <Link key={row.id} href={`/issues/${row.id}`} style={{ textDecoration:"none" }}>
                      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 14px", borderRadius:14, background:"var(--bg3)" }}>
          ...
          <div className="card fade-up d3" style={{ padding:"24px", border:"1px solid rgba(245,158,11,0.25)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:20 }}>
              ...
              <div>
                ...
                <p style={{ fontSize:12, color:"var(--text3)", marginTop:2 }}>Risiko sedang</p>
              </div>
            </div>
            {mediumRiskIssues.length===0
              ...
              : <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  {mediumRiskIssues.map(row=>(
                    <Link key={row.id} href={`/issues/${row.id}`} style={{ textDecoration:"none" }}>
                      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 14px", borderRadius:14, background:"rgba(245,158,11,0.06)", border:"1px solid rgba(245,158,11,0.18)" }}>
                        <p style={{ fontSize:13.5, fontWeight:600, color:"var(--text)" }}>{row.keyword}</p>
          ...
          <div className="card fade-up d4" style={{ padding:"24px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:20 }}>
              ...
              <div>
                <h3 style={{ fontFamily:"'Syne',sans-serif", fontSize:15, fontWeight:700, color:"var(--text)" }}>Issue Aman</h3>
                <p style={{ fontSize:12, color:"var(--text3)", marginTop:2 }}>Risiko rendah</p>
              </div>
            </div>
            {goodIssues.length===0
        ...
        <div className="card fade-up d3" style={{ overflow:"hidden" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"20px 26px", borderBottom:"1px solid var(--border)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              ...
              <h2 style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:15, color:"var(--text)" }}>Issue Terbaru</h2>
          ...
          {recent.length===0
            ? <div style={{ textAlign:"center", padding:"72px 0" }}>
                <Users size={22} style={{ color:"var(--text4)", margin:"0 auto 14px" }} />
                <p style={{ fontWeight:600, color:"var(--text)", fontSize:14 }}>Belum ada issue</p>
                <p style={{ fontSize:12.5, color:"var(--text3)", marginTop:4, marginBottom:20 }}>Mulai tambah keyword untuk monitoring</p>
            ...
            : <table style={{ width:"100%", borderCollapse:"collapse" }}>
                <thead>
                  <tr style={{ background:"var(--bg3)", borderBottom:"1px solid var(--border)" }}>
                    {["Issue / Keyword","Status","Risiko","Tanggal",""].map(h=>(
                    ...
                    <tr key={row.id} style={{ borderBottom:"1px solid var(--border)", transition:"background 0.12s" }}
                      ...
                      <td style={{ padding:"15px 22px" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                          <div style={{ width:36, height:36, borderRadius:"50%", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", background:"linear-gradient(135deg,#38bdf8,#0284c7)", fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:13, color:"#fff" }}>
                            {row.keyword ? row.keyword.charAt(0).toUpperCase() : "?"}
                          </div>
                          <p style={{ fontSize:13.5, fontWeight:600, color:"var(--text)" }}>{row.keyword}</p>
                        </div>
                      </td>
