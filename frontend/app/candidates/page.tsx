...
import { Plus, Trash2, Eye, Loader2, Search, FileSpreadsheet, Download, ChevronDown, ThumbsUp, ThumbsDown, Radio } from "lucide-react";
...
export default function IssuesPage() {
  ...
  async function downloadBulk(riskFilter: string) {
    ...
    if (targets.length === 0) {
      alert(`Tidak ada issue ${riskFilter === "all" ? "" : `risiko ${riskFilter} `}yang sudah selesai analisis.`);
      setDlOpen(false);
      return;
    }
    setDlLoading(true);
    ...
    try {
      ...
      const cats = [
        ...
        { label: "Fraud / Scam Risk", key: "professional_risk", color: [239, 68, 68]  as [number, number, number] },
      ];
      const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
      const W = 210, margin = 18;

      function buildPage(issue: Row, isFirst: boolean) {
        ...
        doc.text("StrappingMedia", margin, 16);
        ...
        doc.text(`${targets.indexOf(issue) + 1} / ${targets.length}`, W - margin, 8, { align: "right" });
        y = 48;

        doc.setFillColor(248, 250, 252); doc.roundedRect(margin, y, W - margin * 2, 22, 3, 3, "F");
        doc.setDrawColor(226, 232, 240); doc.setLineWidth(0.3); doc.roundedRect(margin, y, W - margin * 2, 22, 3, 3, "S");
        doc.setFont("helvetica", "bold"); doc.setFontSize(14); doc.setTextColor(15, 23, 42);
        doc.text(issue.full_name, margin + 5, y + 9);
        doc.setFont("helvetica", "normal"); doc.setFontSize(8); doc.setTextColor(71, 85, 105);
        doc.text(`ID: ${issue.id.slice(0, 8)}...`, margin + 5, y + 17);
        ...
        if (report.assessment_status) {
          ...
          doc.text(isApp ? "KEPUTUSAN: RELEVAN" : "KEPUTUSAN: TIDAK RELEVAN", margin + 5, y + 9);
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.setFont("helvetica", "normal"); doc.setFontSize(7); doc.setTextColor(148, 163, 184);
        doc.text("StrappingMedia — Media Intelligence Platform · Dokumen Rahasia", margin, 291);
      ...
      doc.save(`StrappingMedia_Bulk_${riskSlug[riskFilter]}_${new Date().toISOString().slice(0, 10)}.pdf`);
      ...
      <div style={{ minHeight: "100vh" }} onClick={() => dlOpen && setDlOpen(false)}>
        ...
        <div style={{ background: "var(--bg2)", borderBottom: "1px solid var(--border)", padding: "24px 32px" }}>
          <div style={{ maxWidth: 1160, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
            <div>
              <p style={{ fontFamily: "'Syne',sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--accent)", marginBottom: 6 }}>Media Monitoring</p>
              <h1 style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 30, color: "var(--text)", letterSpacing: "-0.03em" }}>Semua Issue & Keyword</h1>
            </div>
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              <Link href="/candidates/add" className="btn btn-primary"><Plus size={13} /> Tambah Issue</Link>
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1160, margin: "0 auto", padding: "24px 32px" }}>
          ...
          <div style={{ position: "relative", marginBottom: 20 }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Cari issue atau keyword..."
          ...
          {loading ? (
            ...
            <div className="card" style={{ textAlign: "center", padding: "80px 0" }}>
              <div style={{ width: 56, height: 56, borderRadius: 16, background: "var(--bg3)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                <Radio size={24} style={{ color: "var(--text4)" }} />
              </div>
              ...
              <Link href="/candidates/add" className="btn btn-primary" style={{ display: "inline-flex" }}><Plus size={12} /> Tambah Issue</Link>
            </div>
          ) : (
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 820 }}>
                  <thead>
                    <tr style={{ background: "var(--bg3)", borderBottom: "1px solid var(--border)" }}>
                      {["Issue / Keyword", "Status", "Risiko", "Keputusan", "Tanggal", ""].map(h => (
                        ...
                        <tr key={row.id} style={{ borderBottom: "1px solid var(--border)", transition: "background 0.12s" }}
                          ...
                          <td style={{ padding: "14px 18px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                              ...
                              <div>
                                <p style={{ fontSize: 13, fontWeight: 700, color: "var(--text)", margin: 0 }}>{row.full_name}</p>
                          ...
                          <td style={{ padding: "14px 18px" }}>
                            {!isCompleted ? (
                              ...
                              <span style={{
                                ...
                                <ThumbsDown size={11} /> Tidak Relevan
                              </span>
                            )}
                          </td>
                          ...
                          <td style={{ padding: "14px 18px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                              <Link href={`/candidates/${row.id}`} style={{ width: 30, height: 30, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text3)", textDecoration: "none", transition: "all 0.15s" }}
